...
export default class ListView extends cc.Component {
    ...
    onDestroy() {
        ...
        // if (total)
        //     cc.log('-----------------' + t.node.name + '<ListView> destroy node total num. =>', total);
    }
    ...
    //清空滚动层并复位(释放内存)
    removeAllListChildren() {
        //先把内存释放一下
        let t: any = this;
        while (t.content && t.content.childrenCount > 0) {
            t._delSingleItem(this.content.children[this.content.childrenCount - 1]);
        }
        this.numItems = 0
        if (this.content) {
    ...
    checkInited(printLog: boolean = true) {
        if (!this._inited) {
            if (printLog)
                cc.error('ListView initialization not completed!');
            return false;
        }
        return true;
    }
        ...
        if (this._virtual) {
            ...
            if (this._forceUpdate || haveDataChange) {    //如果是强制更新
                if (this.frameByFrameRenderNum > 0) {
                    ...
                    //直接渲染
                    this._lastDisplayData = [];
                    // cc.log('ListView Display Data II::', this.displayData);
     ...
     */
    scrollTo(listId: number, timeInSecond: number = 0, offset: number = null, overStress: boolean = false) {
        let t = this;
        if (!t.checkInited(false))
            return;
