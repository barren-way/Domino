...
@ccclass
export default class RadioHomeLayer extends cc.Component {
    ...
    playRadioAction(): void {
        ...
        let radioData = g.Radio.removeHomeRadioData()
        if (!radioData) return
        this.m_curCountent = radioData.content
        //this.m_contentText.color = this.getRadioTextColor(radioData.rType)
        this.m_contentText.getComponent(cc.Label).string = radioData.content
        ...
        function beginMoveCallback() {
            ...
            let moveLength = textContentSize.width + 20 + this.m_textOrgPos.x * 2
            let moveTime = moveLength / this.m_relativeTime
            let moveToLeft = cc.moveTo(moveTime, cc.v2(-400 - textContentSize.width, this.m_textOrgPos.y))
    ...
    addDefaultSystemRadioData(): void {
        ...
        let defalutData = g.Radio.getDefalutRadioData()
        if (defalutData && defalutData.length <= 0) {
            return
        }
        for (let index = 0; index < defalutData.length; index++) {
            g.Radio.addHomeRadioData(defalutData[index])
        }
    }
