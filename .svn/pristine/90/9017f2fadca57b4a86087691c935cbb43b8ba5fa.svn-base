...
import {Dian21Def} from "./Dian21Def"
...
@ccclass
export default class Dian21 implements IApp {
    ...
    getMsgType(gameid?: any) {
        let msgtype = Dian21Def.MSGTYPE_GAME
        switch (gameid) {
    ...
    getLangJsonName() {
        return Dian21Def.LANG_JSONNAME
    }

    getLangTableName() {
        return Dian21Def.LANG_TABLENAME
    }
    ...
    private static mInstance: Dian21 = null;


    public static getInstance() {
        if (!this.mInstance) {
            this.mInstance = new Dian21()
        }
    ...
    getScript(node, winid) {
        if (node && winid >= 0) {
            return node.getComponent(Dian21Def.SCRIPTS[winid])
        }
    }
}